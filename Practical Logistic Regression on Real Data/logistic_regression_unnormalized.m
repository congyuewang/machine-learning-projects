function [accuracy , w ]=logistic_regression_unnormalized()
...
eta=0.001;
for i=1:200000;
% cross=transpose(x_1_1)*(sigmoid(x_1_1*w)-x_1_y);
cross=transpose(x_1_1)*(1.0./(1.0+exp(-x_1_1*w))-x_1_y);
%value=x_1_1*w;
%disp(value);
w=w-eta*cross;
%tmp=w;
%tmp(1)=[];
error=-sum((x_1_y).*log(sigmoid(x_1_1*w))+(1-x_1_y).*log(1-sigmoid(x_1_1*w)));
%disp(error);
