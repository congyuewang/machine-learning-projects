...
%   class "k" and in bin "j"
    ...
    for i = 1:57
        u = unique(x(:,i));%u is in sorted order
        [a,~] = hist(x(:,i),u);%If xbins is a vector of unevenly spaced values, then hist uses the midpoints between consecutive values as the bin edges.
        a = a(:);
        acum = cumsum(a);
        cumn = 0;
